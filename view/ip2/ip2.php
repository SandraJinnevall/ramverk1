...
<form method="get">
        ...
        <input type="text" value="<?= $currentIP ?>" name="ipadress">
        <input type="submit" value="Validera">
</form>

<h1>Kontrollera IP JSON</h1>
<form action="./CheckjsonController2" method="get">
        <label for="ipjson">Validera och visa ip adress i JSON-format: </label>
        <input type="text" value="<?= $currentIP ?>" name="ipjson" >
        <input type="submit" value="Validera">
</form>
...
<p>Jag återanvände kod från kmom01 att ovan är det ett formulär som skickar ip-adressen tillsammans med get.
  Hämtar upp värdet i min CheckjsonController-klass med funktionen getGet skickar in variablen i mina tre
  olika funktioner tillsammans i en array. De tre funktionerna kollar om det är en validerbar ipadress,
  vilken ipv och om den har en domän. Returnerar tillslut hela array:en vilket blir ett JSON-resultat.
  Det nya i kmom02 är att jag har lagt till två ytterligare funktioner. En som hämtar positionen från den
  inskickade ipadress och den andra hämtar användarens ipadress. För att hämta positionen från ip-adressen
  använder jag http://api.ipstack.com med min egna API nyckel.</p>
